...
const Caustics = (renderer, {
  ...
  const plane = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), new CausticsProjectionMaterial({
    transparent: true,
    color: params.color,
  ...
  const update = () => {
    if (params.frames === Infinity || count++ < params.frames) {
      var _scene$parent;
      if (isVector3(lightSource)) lightDir.copy(lightSource).normalize();else lightDir.copy(ref.worldToLocal(lightSource.getWorldPosition(v)).normalize());
      lightDirInv.copy(lightDir).multiplyScalar(-1);
      (_scene$parent = scene.parent) == null ? void 0 : _scene$parent.matrixWorld.identity();
