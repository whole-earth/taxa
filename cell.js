...
class CellComponent {
  ...
  applyCustomShader(shader) {
    ...
    if (shader instanceof THREE.MeshBasicMaterial) {
      ...
      // CHANGED 1.2 : proper configuration for meshTransmissionMaterial
      this.mesh = this.object
      this.blobChild = this.object.getObjectByName('OUTER')
      this.blobChild.material = shader;
      // CHANGED 1.2 : added gui here, just comment this below to remove
...
const multiplierDistanceControl = 10; // hmm...
...
window.addEventListener('scroll', function () {
  ...
  // console.log("multiplier",multiplier * multiplierValue);
  ...
  scrollTimeout = setTimeout(function () {
    controls.autoRotateSpeed = 0.5;
  }, 100); // reset speed after 100ms of inactivity
...
// CHANGES : I tried the hdr below just to mess around with the lighting. The original soil.hdr one also works pretty great, I have commented that out for now above.
// rgbeLoader.load("assets/environments/soil.hdr", function (texture) {

// rgbeLoader.load("assets/environments/oil-map.hdr", function (texture) {

rgbeLoader.load("https://dl.polyhaven.org/file/ph-assets/HDRIs/hdr/1k/dancing_hall_1k.hdr", function (texture) {
  const pmremGenerator = new PMREMGenerator(cellRender);
  pmremGenerator.compileEquirectangularShader();
  const envMap = pmremGenerator.fromEquirectangular(texture).texture;

  scene.environment = envMap;
  // CHANGES : I add the line below. EquirectangularReflectionMapping on Enviroment mapping and it really gives that extra edge to it
...
// CHANGES : changed the z index on the ribbon to make it appear behind the glass
const loadPromises = [
  new CellComponent("blob-outer_REPLACE.glb", glass, { z: -1 }),
  ...
  const waveShader = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
        ...
        void main() {
            ...
            gl_FragColor = vec4(vNormal * 0.5 + 0.5, 0.1); // Opacity set to 0.3
        }
    `,
