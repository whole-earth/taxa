...
const SpriteAnimator = ({
  ...
  let matRef = spriteMaterial;
  let spriteRef = spriteMesh;
  ...
  const setSpriteTexture = texture => {
  ...
  let displayAsSprite = asSprite !== null && asSprite !== void 0 ? asSprite : true;
  const setDisplayAsSprite = state => {
  ...
  async function loadJsonAndTextureAndExecuteCallback(jsonUrl, textureUrl, callback) {
  ...
  const runAnimation = () => {
    ...
    if (currentFrame > _endFrame) {
      currentFrame = loop ? startFrame !== null && startFrame !== void 0 ? startFrame : 0 : 0;
      if (loop) {
    ...
    const frameOffsetY = 1 / framesV;
    finalValX = flipOffset > 0 ? frameOffsetX * (frameX / originalSizeX) : frameOffsetX * (frameX / originalSizeX) - matRef.map.repeat.x;
  ...
  const update = () => {
    var _spriteData, _matRef;
    if (!((_spriteData = spriteData) != null && _spriteData.frames) || !((_matRef = matRef) != null && _matRef.map)) {
      return;
    }
    if (pause) {
      return;
